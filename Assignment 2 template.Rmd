...
i <- 1; x1 <- x0
p <- numeric(N) #store next xi/iterations-root approximations
for (i in 1:N) {
  print(paste(i,' iteration'))

df.dx  =  genD(func = f, x = x0)$D[1]#   (f(x0+h)-f(x0))/h
if (df.dx==0) break

x1  =  (x0 - (f(x0)/df.dx))

if (is.na(x1) || is.infinite(x1)) break
...
```{r}
start_lambda=0.5
optim(start_lambda,fn=liklihood,sim_data=exp_data,method='L-BFGS-B',lower=0.0001,upper=10000, control=list(fnscale=-1)  )
