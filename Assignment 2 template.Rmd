...
exp_sim=function(n_exp){
  lambda=0.5
  samp_results=c()#numeric()
  for(i in 1:n_exp) {
    xi= runif(1)  
    x = -log(xi)/ lambda 
    samp_results=c(samp_results,x)#exp_res
  }
...
# get derivative function of the above
deriv_liklihood=function(lambda,sim_data=exp_data){
    sx=sum(sim_data)
    n=length(sim_data)
    n / lambda - sx
}
...
deriv_second_liklihood=function(lambda,sim_data=exp_data){
    sx=sum(sim_data)
    n=length(sim_data)
    -n / (lambda^2)
}
...
newton = function(f, f_prime, x0, tol = 1e-10, N = 100) {
  ...
  for (i in 1:N) {
    ...
    fx  = f(x)
    dfx = f_prime(x)
    if (is.na(fx) || is.na(dfx) || abs(dfx) < 1e-12) {
      stop(" Houston, we have a problem, division by zero")
    }
    x_new = x - fx / dfx
    if (is.na(x_new) || is.infinite(x_new)) {
      stop(" infinite vals")
    }
    if (abs(x_new - x) < tol) {
      return(x_new)
...
newton(
  f = deriv_liklihood,
  f_prime = deriv_second_liklihood,
  x0 = 0.5
)
```
...
```{r}
start_lambda=0.5
optim(start_lambda,fn=liklihood,sim_data=exp_data,method='L-BFGS-B',lower=0.0001,upper=Inf, control=list(fnscale=-1)  )
...
toss_to_TT=function(){
  ...
  index_n=c(n)
  flip_result=c() 
  flip_result[1]=flip() # initialize the first value of the flip
  ...
  repeat{
    n=n+1
    flip_result=c(flip_result,flip())
...
mean_flips=round(mean(number_flips_vec));mean_flips
```
